...
bool SaveJPEG(const Path& file, const Image& image) {
    ...
    */
    ...
    row_stride = image.GetWidth() * pix_part_count; /* JSAMPLEs per row in image_buffer */

    /*
    libjpeg принимает строку изображения в виде указателя на массив элементов типа JSAMPLE. 
    ...
    */
    ...
    while (cinfo.next_scanline < cinfo.image_height) {
        ...
        */
        ...
        const Color* line = image.GetLine(scanline);
        
        for (int x = 0; x < w; ++x) {
            image_buffer[x * pix_part_count]     = static_cast<JSAMPLE>(line[x].r);
            image_buffer[x * pix_part_count + 1] = static_cast<JSAMPLE>(line[x].g);
            image_buffer[x * pix_part_count + 2] = static_cast<JSAMPLE>(line[x].b);
        }
